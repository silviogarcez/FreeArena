...
public class CamaraJugador : MonoBehaviour
{
    ...
    private int mouseXSpeedMod = 250;
    private int mouseYSpeedMod = 250;
    ...
    void LateUpdate()
    {
        //var CameraTarget = Camera.main.transform;

        if (Input.GetMouseButton(1))
        {
            x += Input.GetAxis("Mouse X") * mouseXSpeedMod * Time.deltaTime;
            y += Input.GetAxis("Mouse Y") * mouseYSpeedMod * Time.deltaTime;
        }
        ...
        if (Input.GetMouseButton(1))
        {
            ...
            //CameraTarget.localRotation = Quaternion.Euler(-transform.eulerAngles.y, cameraX, 0).normalized;
        }
